...
for df in usa_spending_csvs:
    ...
    statetotals = df.groupby(['recipient_state_name','action_date_fiscal_year'])['total_obligated_amount'].sum()
    statetotals = statetotals.reset_index()
    statetotals['recipient_state_name'] = statetotals['recipient_state_name'].str.title()
    states_per_year = state_spending[['State',listforstate[n]]]
    
    statetotals = pd.merge(statetotals,states_per_year,left_on='recipient_state_name',right_on='State',how='inner')
    print(statetotals.head(5))
    print(statetotals.columns)
    state_totals_by_year_list.append(statetotals)
    
    n-=1

state_fed_spend_by_year = pd.concat(state_totals_by_year_list)
state_fed_spend_by_year = state_fed_spend_by_year.drop(columns='State')
state_fed_spend_by_year['K12PerPupilAnnualSpendingByState_2023'] = state_fed_spend_by_year['K12PerPupilAnnualSpendingByState_2023'].astype(float)
state_fed_spend_by_year['K12PerPupilAnnualSpendingByState_2024'] = state_fed_spend_by_year['K12PerPupilAnnualSpendingByState_2024'].astype(float)
state_fed_spend_by_year['K12PerPupilAnnualSpendingByState_2025'] = state_fed_spend_by_year['K12PerPupilAnnualSpendingByState_2025'].astype(float)
print(state_fed_spend_by_year)